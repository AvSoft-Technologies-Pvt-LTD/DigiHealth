...
const MobileFilterModal = ({ isOpen, onClose, filters, activeFilters, onChange, onReset, onApply }) => {
...
const DynamicTable = ({
  ...
  // Function to get mobile header elements (limited to first 3)
  const getMobileHeaderElements = (row) => {
    const elements = [];
    // Priority 1: Main title (Doctor Name OR Name OR Hospital Name)
    if (doctorNameColumn) {
      elements.push({
        type: 'title',
  ...
  // Function to get elements that should be in card body (those not in header)
