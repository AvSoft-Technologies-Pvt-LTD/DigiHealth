...
const DynamicTable = ({
  ...
  return (
    <div className="space-y-2">
      ...
      {showSearchBar && (
        <div className="flex items-center gap-4">
          {/* Desktop: Filter and Search Layout */}
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">
                      {Object.values(activeFilters).reduce((count, filterValues) => count + (filterValues?.length || 0), 0) > 0 
                        ? `${Object.values(activeFilters).reduce((count, filterValues) => count + (filterValues?.length || 0), 0)} Filter${Object.values(activeFilters).reduce((count, filterValues) => count + (filterValues?.length || 0), 0) !== 1 ? 's' : ''}` 
                        : 'Filter'}
                    </span>
                    {isDesktopFiltersExpanded ? (
  ...
  <div className="relative z-50 mb-4">
    <div className="absolute top-0 left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg p-6">
      ...
      <div className="flex justify-end mt-4">
        <button
          onClick={() => {
            handleDesktopFilterToggle(); // Collapse the filter panel
          }}
        ...
        >
          ...
          {/* Mobile & Tablet: Keep Original Logic */}
          <div className="flex xl:hidden items-center gap-2 w-full">
            <div className="p-2 flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  value={search}
                  onChange={handleSearchChange}
                  className="w-full pl-10 pr-4 py-3 bg-white rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[var(--primary-color)] text-sm"
                />
              </div>
            </div>
            ...
            {filters.length > 0 && (
              <button
                ...
                className="flex items-center justify-center w-10 h-10 bg-[var(--accent-color)] rounded-lg text-white transition-colors flex-shrink-0"
              >
                <Filter className="w-4 h-4" />
              </button>
            )}
      ...
      {/* Desktop Table */}
      <div className={`hidden xl:block bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden relative ${isDesktopFiltersExpanded ? 'mt-32' : ''}`}>
