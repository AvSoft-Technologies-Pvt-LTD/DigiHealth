...
const getNextTokenNumber = () => {
  let t = [];
  try { t = JSON.parse(localStorage.getItem(TOKENS_KEY)) || []; } catch { t = []; }
...
const TokenGenerator = ({ onTokenGenerated }) => {
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8 pt-4 sm:pt-6">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-8 border-l-4 border-[var(--accent-color)]">
            <div className="flex items-center mb-4 sm:mb-6">
              <div className="card-icon card-icon-accent mr-2 sm:mr-4"><Plus className="card-icon-white" size={20} sm:size={24} /></div>
              <h2 className="text-lg sm:text-xl font-bold text-[var(--primary-color)]">Patient Information</h2>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-[var(--primary-color)] flex items-center"><User className="mr-1 sm:mr-2" size={16} />Patient Name *</label>
                  <div className="relative">
                    <input type="text" value={formData.patientName} onChange={e => handleInputChange('patientName', e.target.value)} className={`w-full input-field pl-8 sm:pl-12 ${errors.patientName ? 'shake-red' : ''}`} placeholder="Enter patient name" required />
                    {errors.patientName && <span className="error-text">{errors.patientName}</span>}
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-[var(--primary-color)] flex items-center"><Phone className="mr-1 sm:mr-2" size={16} />Phone Number *</label>
                  <div className="relative">
                    <input type="tel" maxLength={10} pattern="\d{10}" value={formData.phoneNumber} onChange={e => handleInputChange('phoneNumber', e.target.value.replace(/[^\d]/g, '').slice(0,10))} className={`w-full input-field pl-8 sm:pl-12 ${errors.phoneNumber ? 'shake-red' : ''}`} placeholder="Enter 10 digit phone number" required />
                    {errors.phoneNumber && <span className="error-text">{errors.phoneNumber}</span>}
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-[var(--primary-color)] flex items-center"><Building2 className="mr-1 sm:mr-2" size={16} />Department *</label>
                  <div className="relative">
                    <select value={formData.department} onChange={e => handleInputChange('department', e.target.value)} className={`w-full input-field pl-8 sm:pl-12 ${errors.department ? 'shake-red' : ''}`} required>
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-[var(--primary-color)] flex items-center"><Stethoscope className="mr-1 sm:mr-2" size={16} />Select Doctor *</label>
                  <div className="relative">
                    <select value={formData.doctorId} onChange={e => handleInputChange('doctorId', e.target.value)} className={`w-full input-field pl-8 sm:pl-12 ${errors.doctorId ? 'shake-red' : ''}`} disabled={!formData.department} required>
                ...
                <div className="space-y-2 md:col-span-2">
                  <label className="block text-sm font-medium text-[var(--primary-color)] flex items-center"><AlertCircle className="mr-1 sm:mr-2" size={16} />Priority Level</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
                    {priorities.map(priority => (
                      <label key={priority.value} className="cursor-pointer">
                        ...
                        <div className={`p-2 sm:p-3 rounded-lg border-2 transition-all ${formData.priority === priority.value ? `${priority.bg} border-current ${priority.color}` : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
              ...
              {selectedDoctor &&
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-3 sm:p-4 border border-blue-200">
                  <div className="flex items-center space-x-2 sm:space-x-4">
                    <div className="card-icon card-icon-primary"><UserCheck className="card-icon-white" size={16} sm:size={20} /></div>
                    <div>
                      <h4 className="font-semibold text-[var(--primary-color)] text-sm sm:text-base">{selectedDoctor.name}</h4>
                      <p className="text-xs sm:text-sm text-gray-600">{selectedDoctor.degree}</p>
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-[var(--primary-color)] flex items-center"><FileText className="mr-1 sm:mr-2" size={16} />Reason for Visit *</label>
                <div className="relative">
                  <textarea value={formData.reason} onChange={e => handleInputChange('reason', e.target.value)} className={`w-full input-field pl-8 sm:pl-12 ${errors.reason ? 'shake-red' : ''}`} rows={3} placeholder="Describe the reason for visit" required />
                  {errors.reason && <span className="error-text">{errors.reason}</span>}
                </div>
              </div>
              <button type="submit" disabled={isGenerating} className={`w-full btn btn-primary ${isGenerating ? 'animate-pulse-save' : ''}`}>
                {isGenerating ? (<><div className="loader-spinner mr-2"></div>Generating Token...</>) : (<><Plus size={16} sm:size={20} />Generate Token</>)}
              </button>
            </form>
          </div>
        </div>
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-[var(--primary-color)]">
            <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-4 flex items-center"><AlertCircle className="mr-1 sm:mr-2" size={16} sm:size={20} />Next Token</h3>
            <div className="text-center">
              <div className="text-2xl sm:text-4xl font-black text-[var(--primary-color)] mb-1 sm:mb-2">T{nextTokenNumber.toString().padStart(3, '0')}</div>
              <p className="text-xs sm:text-sm text-gray-600">Token Number</p>
            </div>
          </div>
          {formData.department &&
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
              <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-4 flex items-center"><Stethoscope className="mr-1 sm:mr-2" size={16} sm:size={20} />Available Doctors</h3>
              <div className="space-y-2 sm:space-y-3 max-h-64 overflow-y-auto">
                {availableDoctors.map(doctor => (
                  <div key={doctor.id} className={`p-2 sm:p-3 rounded-lg border transition-all cursor-pointer ${formData.doctorId === doctor.id ? 'border-[var(--accent-color)] bg-green-50' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`} onClick={() => handleInputChange('doctorId', doctor.id)}>
                    <div className="font-medium text-[var(--primary-color)] text-xs sm:text-sm">{doctor.name}</div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-4 flex items-center"><Building2 className="mr-1 sm:mr-2" size={16} sm:size={20} />Departments</h3>
            <div className="grid grid-cols-2 gap-1 sm:gap-2 text-xs sm:text-sm">
              {departments.slice(0, 8).map(dept => (
                <div key={dept.value} className={`flex items-center gap-1 sm:gap-2 p-1 sm:p-2 rounded transition-all cursor-pointer ${formData.department === dept.value ? 'bg-[var(--accent-color)] text-white' : 'bg-gray-50 hover:bg-gray-100'}`} onClick={() => handleInputChange('department', dept.value)}>
                  <span>{dept.icon}</span>
