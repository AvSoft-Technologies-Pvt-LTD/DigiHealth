...
const QueueManagement = ({ tokens, onTokensUpdate }) => {
  ...
  const handleStatusChange = (tokenId, newStatus) => {
    const updated = allTokens.map(t => t.id === tokenId ? { ...t, status: newStatus } : t);
    ...
    if (newStatus === 'called' && token) setTimeout(() => announcePatient(token), 200);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Queue Management System</h1>
            <p className="text-sm sm:text-base text-gray-600 mt-1">Manage and monitor patient queue status</p>
          </div>
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium">Voice Announcements:</label>
            <button
              onClick={() => setVoiceEnabled(!voiceEnabled)}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${voiceEnabled ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-600'}`}
            >
              <Volume2 size={16} />
              {voiceEnabled ? 'ON' : 'OFF'}
            </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          <div className="card-stat card-border-primary">
            <div className="flex items-center justify-between">
              ...
              <Users className="text-[var(--primary-color)]" size={20} />
            </div>
          </div>
        ...
        {/* Token Queue Table (Responsive) */}
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
          <h3 className="text-lg sm:text-xl font-bold mb-4">Token Queue</h3>
          ...
          <div className="space-y-3 sm:space-y-0 sm:flex sm:items-center sm:gap-4 mb-4 sm:mb-6">
            <div className="w-full sm:w-auto">
              <input
                ...
                placeholder="Search tokens, patients, departments..."
                className="input-field w-full sm:w-64"
              />
            </div>
            <div className="flex items-center gap-2">
              <Filter className="text-[var(--primary-color)] text-base" />
              ...
              {filters.department && (
                <button onClick={() => clearFilter('department')} className="text-red-500 hover:text-red-700" title="Clear filter">
                  <X size={16} />
                </button>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              {filters.status && (
                <button onClick={() => clearFilter('status')} className="text-red-500 hover:text-red-700" title="Clear filter">
                  <X size={16} />
                </button>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              {filters.priority && (
                <button onClick={() => clearFilter('priority')} className="text-red-500 hover:text-red-700" title="Clear filter">
                  <X size={16} />
                </button>
              )}
            </div>
            ...
            <table className="table-container w-full">
              <thead>
                <tr className="table-head text-left">
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Token</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Patient</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Department</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Assigned Doctor</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Priority</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Status</th>
                  <th className="px-3 py-2 sm:px-4 sm:py-3">Generated At</th>
              ...
              <tbody className="table-body">
                {filteredTokens.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-3 py-6 sm:px-4 sm:py-8 text-center text-gray-500">No tokens found matching your criteria</td>
                  </tr>
                ) : (
                  filteredTokens.map((token) => (
                    <tr key={token.id} className="border-t hover:bg-gray-50 transition-colors">
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <span className="font-bold text-[var(--primary-color)]">{token.tokenNumber}</span>
                      </td>
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <div>
                      ...
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <span className="capitalize">
                          {departments.find(d => d.value === token.department)?.label || token.department}
                        </span>
                      </td>
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <span className="text-sm text-gray-700">{token.doctorName || '-'}</span>
                      </td>
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${token.priority === 'emergency' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                          {token.priority}
                        </span>
                      </td>
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <select
                          ...
                          className={`border rounded px-2 py-1 text-sm font-medium ${token.status === 'waiting' ? 'border-yellow-300 bg-yellow-50 text-yellow-800' : token.status === 'called' ? 'border-blue-300 bg-blue-50 text-blue-800' : token.status === 'completed' ? 'border-green-300 bg-green-50 text-green-800' : 'border-red-300 bg-red-50 text-red-800'}`}
                        >
                      ...
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <div className="text-sm">
                          <div>{new Date(token.generatedAt).toLocaleTimeString()}</div>
                          <div className="text-xs text-gray-500">{new Date(token.generatedAt).toLocaleDateString()}</div>
                        </div>
                      </td>
                      <td className="px-3 py-2 sm:px-4 sm:py-3">
                        <span className="text-sm text-gray-600 truncate max-w-32 block">{token.reason}</span>
                      </td>
                    </tr>
                  ))
                )}
          ...
          <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm">
              <span className="text-gray-600 mb-2 sm:mb-0">Showing {filteredTokens.length} of {allTokens.length} tokens</span>
