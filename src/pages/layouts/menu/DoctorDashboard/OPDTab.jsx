    ...
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-2 sm:p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="flex flex-col relative w-full max-w-4xl max-h-[95vh] rounded-xl bg-white shadow-xl overflow-hidden" initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <div className="bg-white border-t p-4 flex justify-end">
          <button onClick={() => onEdit(patient)} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Edit Patient</button>
        </div>
      </motion.div>
    </motion.div>
  );
...
const OpdTab = forwardRef(({ doctorName, masterData, location, setTabActions }, ref) => {
  ...
  const handleFormChange = (data) => {
    if (data.sameAsPermAddress && data.addressPerm) data.addressTemp = data.addressPerm;
    setFormData(data);
  };
  ...
  const generatePatientBasicFields = () => [
    ...
    { name: "bloodGroup", label: "Blood Group", type: "select", options: masterData.bloodGroups },
