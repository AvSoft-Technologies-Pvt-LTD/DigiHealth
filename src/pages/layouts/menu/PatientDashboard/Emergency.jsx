...
const Emergency = () => {
  ...
  const generateSuggestions = (query) => {
    ...
    data.ambulanceServices.forEach((ambulance) => {
      const nameMatches = ambulance.serviceName
        .toLowerCase()
        .includes(queryLower);
      ...
      const categoryMatches = ambulance.category
        .toLowerCase()
        .includes(queryLower);
  ...
  const searchAmbulances = (query = searchQuery) => {
    ...
    setTimeout(() => {
      ...
      let results = data.ambulanceServices.filter((ambulance) => {
        const nameMatch = ambulance.serviceName
          .toLowerCase()
          .includes(queryLower);
        const locationMatch =
        ...
        const categoryMatch = ambulance.category
          .toLowerCase()
          .includes(queryLower);
      ...
      if (results.length === 0) {
        toast.info(
          `No ambulances found for "${query}". Try different search terms.`
        );
      } else {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              ...
              >
                {searchLoading ? (
                  <Lucide.Loader2 className="animate-spin size-4 sm:size-5" size={20} />
                ) : (
                ...
                >
                  <div className="p-4 sm:p-6">
                    ...
                    <div className="flex gap-1.5 sm:gap-2">
                      ...
                      >
                        <Lucide.Phone size={14} className="sm:size-4" /> Call Now
                      </button>
                      <button
                        onClick={() =>
                          toast.success(
      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="bg-gradient-to-r from-[#01B07A] to-[#1A223F] rounded-xl p-4 sm:p-6 mb-4 sm:mb-6 text-white">
          <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 rounded-full flex items-center justify-center">
              <Lucide.CheckCircle className="text-white size-5 sm:size-6" size={24} />
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4">
              <Lucide.Ambulance className="text-red-600 size-5 sm:size-6" size={24} />
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4">
              <Lucide.Calendar className="text-blue-600 size-5 sm:size-6" size={24} />
              <h4 className="text-base sm:text-lg font-semibold text-gray-800">
                Schedule & Location
              </h4>
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
          <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4">
            <Lucide.Package className="text-purple-600 size-5 sm:size-6" size={24} />
            <h4 className="text-base sm:text-lg font-semibold text-gray-800">
              Equipment & Billing
            </h4>
          </div>
      ...
      {showPaymentGateway ? (
        ...
        <div className="max-w-full sm:max-w-6xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              {["Details", "Confirm"].map((stepName, index) => (
                <React.Fragment key={index}>
                  <div className="flex flex-col items-center">
                    ...
                    >
                      {step > index ? (
                        <Lucide.CheckCircle2 className="size-4 sm:size-5" size={20} />
                      ) : (
          ...
          <div className="px-4 py-3 sm:px-6 sm:py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-3">
            ...
            <div className={`flex gap-2.5 sm:gap-3 ${step === 0 ? "ml-auto" : ""}`}>
              {step === 1 ? (
                <>
                  <button
                  ...
                  {calculateEquipmentTotal() > 0 && (
                    ...
                    >
                      <Lucide.CreditCard className="size-3.5 sm:size-4" size={16} /> Pay Now ₹
                      {calculateEquipmentTotal()}
                    </button>
                  )}
                </>
